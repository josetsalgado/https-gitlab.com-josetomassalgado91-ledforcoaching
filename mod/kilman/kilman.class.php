...
class kilman {
    ...
    /**
     * @var \mod_kilman\question\base[] $quesitons
     */
    public $questions = [];
     ...
     *
     */
    public function __construct($id = 0, $kilman = null, &$course, &$cm, $addquestions = true) {
        global $DB;

        if ($id) {
            $kilman = $DB->get_record('kilman', array('id' => $id));
        }

        if (is_object($kilman)) {
            $properties = get_object_vars($kilman);
        ...
        $this->course = $course;
        $this->cm = $cm;
        // When we are creating a brand new kilman, we will not yet have a context.
        ...
        // Load the capabilities for this user and kilman, if not creating a new one.
        if (!empty($this->cm->id)) {
            $this->capabilities = kilman_load_capabilities($this->cm->id);
        }
    }
    ...
    public function add_survey($sid = 0, $survey = null) {
        ...
        if ($sid) {
            $this->survey = $DB->get_record('kilman_survey', array('id' => $sid));
    ...
    public function add_questions($sid = false) {
        ...
        $params = [$sid, 'n'];
        if ($records = $DB->get_records_select('kilman_question', $select, $params, 'position')) {
            $sec = 1;
            $isbreak = false;
    ...
    /**
     * Add the renderer to the kilman object.
    ...
    /**
     * Add the templatable page to the kilman object.
    ...
    public function view() {
        ...
        $PAGE->requires->js_init_call('M.mod_kilman.init_attempt_form', null, false, kilman_get_js_module());
        ...
        if (!$this->capabilities->view) {
            ...
            if (data_submitted() && confirm_sesskey() && isset($viewform->submit) && isset($viewform->submittype) &&
                ...
                if ($this->grade != 0) {
                    $kilman = new stdClass();
                    $kilman->id = $this->id;
                    $kilman->name = $this->name;
                    $kilman->grade = $this->grade;
                    $kilman->cmidnumber = $this->cm->idnumber;
                    $kilman->courseid = $this->course->id;
                    kilman_update_grades($kilman, $quser);
                }
                ...
                $params = array(
                    ...
                    'anonymous' => $anonymous,
                    'other' => array('kilmanid' => $kilman->id)
                );
                $event = \mod_kilman\event\attempt_submitted::create($params);
                $event->trigger();
    ...
    public function view_all_responses($resps) {
        ...
        if ($resps) {
            ...
            $this->page->add_to_page('responses', $this->renderer->all_response_output(get_string('noresponses', 'kilman')));
        }
    ...
    public function user_can_take($userid) {
        ...
        if (!$this->is_active() || !$this->user_is_eligible($userid)) {
            return false;
        } else if ($this->qtype == kilmanUNLIMITED) {
            return true;
        } else if ($userid > 0) {
    ...
    public function user_time_for_new_attempt($userid) {
        ...
        $params = ['kilmanid' => $this->id, 'userid' => $userid, 'complete' => 'y'];
        if (!($attempts = $DB->get_records('kilman_response', $params, 'submitted DESC'))) {
            return true;
        }
    ...
    public function can_view_response($rid) {
        ...
        if (!empty($rid)) {
            $response = $DB->get_record('kilman_response', array('id' => $rid));
    ...
    /**
     * True if the user can view the responses to this kilman, and there are valid responses.
    ...
    /**
     * True if the user can view all of the responses to this kilman any time, and there are valid responses.
     * @param null $usernumresp
     * @return bool
     * @throws coding_exception
     */
    ...
    public function count_submissions($userid = false, $groupid = 0) {
        ...
        // Since submission can be across kilmans in the case of public kilmans, need to check the realm.
    ...
    /**
     * Get the requested responses for this kilman.
     *
    ...
    /**
     * Check if current kilman has dependencies set and any question has dependencies.
     *
    ...
    public function print_survey($userid = false, $quser) {
        ...
        // If student saved a "resume" kilman OR left a kilman unfinished
        ...
        if (!empty($formdata->submit)) {
            ...
            if (isset($SESSION->kilman->end) && $SESSION->kilman->end == true) {
                return;
            }
        ...
        // Save each section 's $formdata somewhere in case user returns to that page when navigating the kilman.
            ...
            if ($msg) {
                ...
                if ($this->has_dependencies()) {
                    while (!$this->eligible_questions_on_page($formdata->sec, $formdata->rid)) {
                        $this->response_delete($formdata->rid, $formdata->sec);
                        $formdata->sec++;
                        // We have reached the end of kilman on a page without any question left.
                        if ($formdata->sec > $numsections) {
                            $SESSION->kilman->end = true; // End of kilman reached on a no questions page.
                            break;
                        }
                    }
                }
        ...
        if (!empty($formdata->prev)) {
            ...
            // unlock kilman->end to allow navigate back to previous page.
            if (isset($SESSION->kilman->end) && ($SESSION->kilman->end == true)) {
                $SESSION->kilman->end = false;
                $formdata->sec--;
            }
        ...
        if ($rid) {
            ...
            if ($resp = $DB->get_record('kilman_response', array('id' => $rid))) {
                if ($this->respondenttype == 'fullname') {
                    ...
                    $event = \mod_kilman\event\response_viewed::create($params);
                    $event->trigger();
                }
            }
        }
    ...
    public function survey_print_render($message = '', $referer = '', $courseid, $rid = 0, $blankkilman = false) {
        ...
        if (!empty($rid)) {
            ...
            $this->view_response($rid, $referer, $blankkilman);
            return;
        }
    ...
    public function survey_update($sdata) {
        ...
        if (empty($this->survey->id)) {
            ...
            $this->survey->id = $DB->insert_record('kilman_survey', $record);
            ...
            $name = $DB->get_field('kilman_survey', 'name', array('id' => $this->survey->id));

            // Trying to change survey name.
            if (trim($name) != trim(stripslashes($sdata->name))) {  // $sdata will already have slashes added to it.
                $count = $DB->count_records('kilman_survey', array('name' => $sdata->name));
                if ($count != 0) {
                    $errstr = get_string('errnewname', 'kilman');  // TODO: notused!
                    return(false);
                }
    ...
    public function survey_copy($owner) {
        ...
        // Create new survey.
        if (!($newsid = $DB->insert_record('kilman_survey', $survey))) {
            return(false);
        }
        ...
        foreach ($this->questions as $question) {
            ...
            foreach ($question->choices as $key => $choice) {
                ...
                if (!$newcid = $DB->insert_record('kilman_quest_choice', $choice)) {
                    return(false);
                }
        ...
        $dependquestions = $DB->get_records('kilman_dependency', ['surveyid' => $this->survey->id], 'questionid');
        foreach ($dependquestions as $dquestion) {
            ...
            $DB->insert_record('kilman_dependency', $record);
        }

        // Replicate any feedback data.
        // TODO: Need to handle image attachments (same for other copies above).
        $fbsections = $DB->get_records('kilman_fb_sections', ['surveyid' => $this->survey->id], 'id');
        foreach ($fbsections as $fbsid => $fbsection) {
            ...
            unset($fbsection->id);
            $newfbsid = $DB->insert_record('kilman_fb_sections', $fbsection);
            $feedbackrecs = $DB->get_records('kilman_feedback', ['sectionid' => $fbsid], 'id');
            foreach ($feedbackrecs as $feedbackrec) {
                $feedbackrec->sectionid = $newfbsid;
                unset($feedbackrec->id);
                $DB->insert_record('kilman_feedback', $feedbackrec);
            }
        }
    ...
    private function response_commit($rid) {
        ...
        return $DB->update_record('kilman_response', $record);
    }
    ...
    private function get_response($userid, $rid = 0) {
        ...
        if ($rid != 0) {
            ...
            if ($records = $DB->get_records('kilman_response', $params, 'submitted DESC', 'id,kilmanid', 0, 1)) {
                $rec = reset($records);
                return $rec->id;
            } else {
                return '';
    ...
    private function response_select_max_sec($rid) {
        ...
        $max = $DB->count_records_select('kilman_question', $select, $params) + 1;
    ...
    private function submission_notify($rid) {
        ...
        if (isset($this->survey)) {
            if (isset($this->survey->email)) {
                ...
                $email = $DB->get_field('kilman_survey', 'email', ['id' => $this->survey->id]);
            }
        } else {
            $email = '';
        }
    ...
    private function send_submission_notifications($rid) {
        ...
        if ($notifyusers = $this->get_notifiable_users($USER->id)) {
            ...
            $info->posttext = get_string($langstringtext, 'kilman', $info);
            $info->posthtml = '<p>' . get_string($langstringhtml, 'kilman', $info) . '</p>';
            if (!empty($message)) {
    ...
    private function get_full_submission_for_notifications($answers) {
        ...
        foreach ($responses as $response) {
            ...
            $message .= get_string('answers', 'kilman') . ":<br />\n";
    ...
    public function response_insert($section, $rid, $userid, $resume = false) {
        ...
        if (empty($rid)) {
            ...
            $record->kilmanid = $this->id;
            $record->userid = $userid;
            $rid = $DB->insert_record('kilman_response', $record);
        } else {
            $record->id = $rid;
            $DB->update_record('kilman_response', $record);
        }
        if ($resume) {
            ...
            $params = array(
                ...
                'relateduserid' => $userid,
                'anonymous' => $anonymous,
                'other' => array('kilmanid' => $this->id)
            );
            $event = \mod_kilman\event\attempt_saved::create($params);
            $event->trigger();
        }
    ...
    private function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $values = \mod_kilman\response\boolean::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_single (radio button or dropdown).
        $values += \mod_kilman\response\single::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_multiple.
        $values += \mod_kilman\response\multiple::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_rank.
        $values += \mod_kilman\response\rank::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_text.
        $values += \mod_kilman\response\text::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);

        // Response_date.
        $values += \mod_kilman\response\date::response_select($rid, $col, $csvexport, $choicecodes, $choicetext);
    ...
    public function survey_results_navbar_alpha($currrid, $currentgroupid, $cm, $byresponse) {
        ...
        // Is this kilman set to fullname or anonymous?
    ...
    /**
     * Return all the fields to be used for users in kilman sql.
     *
     * @author: Guy Thomas
     * @return array|string
     */
    ...
    protected function get_survey_all_responses($rid = '', $userid = '', $groupid = false, $showincompletes = 0) {
        ...
        // If a kilman is "public", and this is the master course, need to get responses from all instances.
        if ($this->survey_is_public_master()) {
            $qids = array_keys($DB->get_records('kilman', ['sid' => $this->sid], 'id'));
        } else {
            $qids = $this->id;
        }

        foreach ($uniquetypes as $type) {
            $question = \mod_kilman\question\base::question_builder($type);
    ...
    protected function process_csv_row(array &$row, stdClass $resprow, $currentgroupid, array &$questionsbyposition, $nbinfocols, $numrespcols, $showincompletes = 0) {
        ...
        if ($config === null) {
            $config = get_config('kilman', 'downloadoptions');
        }
        ...
        // Moodle:
        // Get the course name that this kilman belongs to.
        if (!$this->survey_is_public()) {
            $courseid = $this->course->id;
        ...
        if ($isanonymous) {
            ...
            $fullname = get_string('anonymous', 'kilman') . $anonumap[$user->id];
            $username = '';
            $uid = '';
        } else {
            $uid = $user->id;
        ...
        if (in_array('submitted', $options)) {
            ...
            $submitted = date(get_string('strfdateformatcsv', 'kilman'), $resprow->submitted);
    ...
    public function generate_csv($rid = '', $userid = '', $choicecodes = 1, $choicetext = 0, $currentgroupid, $showincompletes = 0) {
        ...
        $output = array();
        $stringother = get_string('other', 'kilman');
        ...
        foreach ($options as $option) {
            if (in_array($option, array('response', 'submitted', 'id'))) {
                $columns[] = get_string($option, 'kilman');
                $types[] = 0;
            } else {
        ...
        if (count(array_intersect($choicetypes, $uniquetypes)) > 0) {
            ...
            $choicesql = "
                SELECT DISTINCT c.id as cid, q.id as qid, q.precise AS precise, q.name, c.content
                  FROM {kilman_question} q
                  JOIN {kilman_quest_choice} c ON question_id = q.id
                        ...
                        foreach ($choices as $choice) {
                            ...
                            if (preg_match("/^[0-9]{1,3}=/", $content, $ndd)) {
                                ...
                                if ($osgood) {
                                    ...
                                    $contents = kilman_choice_values($contentleft);
                                    if ($contents->title) {
                                        $contentleft = $contents->title;
                                    }
                                    $contents = kilman_choice_values($contentright);
                                    if ($contents->title) {
        ...
        if (is_array($questions)) {
            ...
            foreach ($questions as $question) {
                ...
                if ($question->id == $movequestion->id) {
                    ...
                    $DB->update_record("kilman_question", $movequestion);
                    continue;
                }
                $question->position = $index;
                $DB->update_record("kilman_question", $question);
                $index++;
            }
            return true;
        }
    ...
    public function response_analysis($rid, $resps, $compare, $isgroupmember, $allresponses, $currentgroupid, $filteredsections = null) {
        ...
        if ($this->survey->feedbackscores) {
            ...
            if ($compare) {
                ...
                $table->head = [get_string('feedbacksection', 'kilman'), $groupname];
            }
        }
