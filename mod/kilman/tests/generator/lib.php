...
use mod_kilman\generator\question_response,
    mod_kilman\generator\question_response_rank,
    mod_kilman\question\base;

global $CFG;
require_once($CFG->dirroot.'/mod/kilman/locallib.php');

class mod_kilman_generator extends testing_module_generator {
     ...
     * @return array
     */
    public function kilmans() {
        return $this->kilmans;
    }

    /**
     * Create a kilman activity.
    ...
    public function create_instance($record = null, array $options = null) {
        ...
        foreach ($defaultkilmansettings as $name => $value) {
        ...
        $cm = get_coursemodule_from_instance('kilman', $instance->id);
        $course = get_course($cm->course);
        $kilman = new kilman(0, $instance, $course, $cm, false);

        $this->kilmans[$instance->id] = $kilman;

        return $kilman;
    }

    /**
     * Create a survey instance with data from an existing kilman object.
     * @param object $kilman
     * @param array $options
     * @return int
     */
    public function create_content($kilman, $record = array()) {
        global $DB;

        $survey = $DB->get_record('kilman_survey', array('id' => $kilman->sid), '*', MUST_EXIST);
        foreach ($record as $name => $value) {
            $survey->{$name} = $value;
        }
        return $kilman->survey_update($survey);
    }
     ...
     * @return \mod_kilman\question\base the question object
     */
    public function create_question(kilman $kilman, $record = null, $data = null) {
        global $DB;
        ...
        $record['position'] = count($kilman->questions);
        ...
        if ($typeid !== QUESPAGEBREAK && $typeid !== QUESSECTIONTEXT) {
            $qtype = $DB->get_record('kilman_question_type', ['id' => $typeid]);
            if (!$qtype) {
        ...
        // Add the question.
        $record->id = $DB->insert_record('kilman_question', $record);

        $question = \mod_kilman\question\base::question_builder($record->type_id, $record->id, $record);
        ...
        // Update kilman.
        $kilman->add_questions();

        return $question;
    }

    /**
     * Create a kilman with questions and response data for use in other tests.
     */
    public function create_test_kilman($course, $qtype = null, $questiondata = array(), $choicedata = null) {
        $kilman = $this->create_instance(array('course' => $course->id));
        $cm = get_coursemodule_from_instance('kilman', $kilman->id);
        if ($qtype !== null) {
            $questiondata['type_id'] = $qtype;
            $questiondata['surveyid'] = $kilman->sid;
            $questiondata['name'] = isset($questiondata['name']) ? $questiondata['name'] : 'Q1';
            $questiondata['content'] = isset($questiondata['content']) ? $questiondata['content'] : 'Test content';
            $this->create_question($kilman, $questiondata, $choicedata);
        }
        $kilman = new kilman($kilman->id, null, $course, $cm, true);
        return $kilman;
    }
     ...
     */
    public function create_question_response($kilman, $question, $respval, $userid = 1, $section = 1) {
        global $DB;
        $currentrid = 0;
        $_POST['q'.$question->id] = $respval;
        $responseid = $kilman->response_insert($section, $currentrid, $userid);
        $this->response_commit($kilman, $responseid);
        return $DB->get_record('kilman_response', array('id' => $responseid));
    }

    /**
     * Need to create a method to access a private kilman method.
     * TO DO - may not need this with above "TO DO".
     */
    private function response_commit($kilman, $responseid) {
        $method = new ReflectionMethod('kilman', 'response_commit');
        $method->setAccessible(true);
        return $method->invoke($kilman, $responseid);
    }
     ...
     *
     * @param \mod_kilman\question\base $question
     * @param stdClass $data
     */
    ...
    protected function add_response_choice($questionresponse, $responseid) {
        ...
        $question = $DB->get_record('kilman_question', ['id' => $questionresponse->questionid]);
        ...
        if ($qtype === QUESCHOOSE || $qtype === QUESRADIO || $qtype === QUESDROP || $qtype === QUESCHECK || $qtype === QUESRATE) {
            if (is_int($questionresponse->response)) {
                ...
                $rs = $DB->get_records_sql("SELECT * FROM {kilman_quest_choice} $select", $params, 0, 1);
                $choice = reset($rs);
                if (!$choice) {
            ...
            if ($qtype == QUESRATE) {
                $DB->insert_record('kilman_response_rank', [
                ...
                if ($qtype === QUESCHOOSE || $qtype === QUESRADIO || $qtype === QUESDROP) {
                    $instable = 'kilman_resp_single';
                } else if ($qtype === QUESCHECK) {
                    $instable = 'kilman_resp_multiple';
                }
            ...
            $DB->insert_record('kilman_response_text', [
                    'response_id' => $responseid,
                    'question_id' => $questionresponse->questionid,
    ...
    public function create_response($record = null, $questionresponses, $complete = true) {
        ...
        if (!isset($record['kilmanid'])) {
            throw new coding_exception('kilmanid must be present in phpunit_util::create_response() $record');
        }
        ...
        // Add the response.
        $record['id'] = $DB->insert_record('kilman_response', $record);
    ...
    /**
     * @param kilman $kilman
     * @param \mod_kilman\question\base[] $questions
     * @param $userid
     * @param $complete
     * @return stdClass
     * @throws coding_exception
     */
    public function generate_response($kilman, $questions, $userid, $complete = true) {
        $responses = [];
        foreach ($questions as $question) {
            ...
            switch ($question->type_id) {
                ...
                case QUESDATE :
                    ...
                    $dateformat = get_string('strfdate', 'kilman');
        ...
        return $this->create_response(['kilmanid' => $kilman->id, 'userid' => $userid], $responses, $complete);
    }

    public function create_and_fully_populate($coursecount = 4, $studentcount = 20, $kilmancount = 2,
            $questionspertype = 5) {
        global $DB;
        ...
        // Create kilmans in each course.
        $qname = 1000;
        for ($q = 0; $q < $kilmancount; $q++) {
            foreach ($courses as $course) {
                $kilman = $qdg->create_instance(['course' => $course->id]);
                $kilmans[] = $kilman;
                $questions = [];
                ...
                foreach ($students as $student) {
                    ...
                    if ($count < $studentcount) {
                        $qdg->generate_response($kilman, $questions, $student->id);
                    } else {
                        $qdg->generate_response($kilman, $questions, $student->id, false);
                    }
                    $count++;
                }
            }
