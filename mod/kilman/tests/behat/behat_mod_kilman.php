...
/**
 * Steps definitions related with the kilman activity.
 *
 * @package    mod_kilman
 * @category   test
...
;
/**
 * kilman-related steps definitions.
 *
 * @package    mod_kilman
 * @category   test
 * @copyright  2016 Mike Churchward - Poet Open Source
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class behat_mod_kilman extends behat_base {

    /**
     * Adds a question to the kilman with the provided data.
     *
    ...
    /**
     * Adds a questions and responses to the kilman with the provided name.
     *
     * @Given /^"([^"]*)" has questions and responses$/
     *
     * @param string $kilmanname The name of an existing kilman.
     */
    public function has_questions_and_responses($kilmanname) {
        global $DB;

        if (!$kilman = $DB->get_record('kilman', array('name' => $kilmanname), 'id,sid')) {
            throw new ExpectationException('Invalid kilman name specified.', $this->getSession());
        }

        if (!$DB->record_exists('kilman_survey', array('id' => $kilman->sid))) {
            throw new ExpectationException('kilman survey does not exist.', $this->getSession());
        }

        $this->add_question_data($kilman->sid);
        $this->add_response_data($kilman->id, $kilman->sid);
    }

    /**
     * Adds a question data to the given survey id.
     *
     * @param int $sid The id field of an existing kilman_survey record.
     * @return null
     */
    private function add_question_data($sid) {
        ...
        $this->add_data($questiondata, 'kilman_question', 'questionmap');
        $this->add_data($choicedata, 'kilman_quest_choice', 'choicemap', array('questionmap' => 'question_id'));
    }

    /**
     * Adds response data to the given kilman and survey id.
     *
     * @param int $qid The id field of an existing kilman record.
     * @param int $sid The id field of an existing kilman_survey record.
     * @return null
     */
    private function add_response_data($qid, $sid) {
        ...
        $responses = array(
            array("id", "kilmanid", "submitted", "complete", "grade", "userid"),
        ...
        $this->add_data($responses, 'kilman_response', 'responsemap');
        ...
        $this->add_data($responsebool, 'kilman_response_bool', '',
        ...
        $this->add_data($responsedate, 'kilman_response_date', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id'));
        ...
        $this->add_data($responseother, 'kilman_response_other', '',
        ...
        $this->add_data($responserank, 'kilman_response_rank', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
        ...
        $this->add_data($respmultiple, 'kilman_resp_multiple', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
        ...
        $this->add_data($respsingle, 'kilman_resp_single', '',
            array('responsemap' => 'response_id', 'questionmap' => 'question_id', 'choicemap' => 'choice_id'));
    }
