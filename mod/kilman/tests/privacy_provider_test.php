...
/**
 * Privacy test for the mod kilman.
 *
 * @package    mod_kilman
...
use \mod_kilman\privacy\provider;
 ...
 * @group      mod_kilman
 */
class mod_kilman_privacy_testcase extends \core_privacy\tests\provider_testcase {
    /**
     * Tests set up.
     */
    ...
    public function test_get_contexts_for_userid() {
        ...
        $qdg = $dg->get_plugin_generator('mod_kilman');
        $qdg->create_and_fully_populate(1, 1, 1, 1);
        $user = $DB->get_record('user', ['firstname' => 'Testy']);
        $kilmans = $qdg->kilmans();
        $kilman = current($kilmans);
        list ($course, $cm) = get_course_and_cm_from_instance($kilman->id, 'kilman', $kilman->course);
    ...
    /**
     * Test that only users with a kilman context are fetched.
     */
    public function test_get_users_in_context() {
        ...
        $userlist = new \core_privacy\local\request\userlist($cmcontext, 'mod_kilman');
        ...
        $userlist = new \core_privacy\local\request\userlist(context_system::instance(), 'mod_kilman');
        provider::get_users_in_context($userlist);
    ...
    public function test_export_user_data() {
        ...
        $approvedlist = new core_privacy\local\request\approved_contextlist($user, 'mod_kilman', [$cmcontext->id]);
        provider::export_user_data($approvedlist);
        $data = $writer->get_data([]);

        $this->assertContains($kilman->name, strip_tags($data->name));
        $this->assertEquals($kilman->intro, strip_tags($data->intro));
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $this->assertCount(2, $DB->get_records('kilman_response', []));

        // Delete everything for the context.
        provider::delete_data_for_all_users_in_context($cmcontext);
        $this->assertCount(0, $DB->get_records('kilman_response', []));
    }
    ...
    public function test_delete_data_for_user() {
        ...
        $approvedlist = new \core_privacy\local\request\approved_contextlist($user, 'kilman_response', [$cmcontext->id]);
        provider::delete_data_for_user($approvedlist);

        $this->assertCount(0, $DB->get_records('kilman_response', ['userid' => $user->id]));

        // Get all accounts. There should be one.
        $this->assertCount(1, $DB->get_records('kilman_response', []));
    }
    ...
    public function test_delete_data_for_users() {
        ...
        $approveduserlist = new \core_privacy\local\request\approved_userlist($cmcontext, 'kilman', [$user->id, $user3->id]);

        // Get all accounts. There should be three.
        $this->assertCount(3, $DB->get_records('kilman_response', []));
