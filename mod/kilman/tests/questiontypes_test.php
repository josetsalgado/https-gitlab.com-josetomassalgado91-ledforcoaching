...
/**
 * Unit tests for {@link kilman_questiontypes_testcase}.
 * @group mod_kilman
 */
class mod_kilman_questiontypes_testcase extends advanced_testcase {
    public function test_create_question_checkbox() {
        $this->create_test_question_with_choices(QUESCHECK,
            '\\mod_kilman\\question\\check', array('content' => 'Check one'));
    }

    public function test_create_question_date() {
        $this->create_test_question(QUESDATE, '\\mod_kilman\\question\\date', array('content' => 'Enter a date'));
    }

    public function test_create_question_dropdown() {
        $this->create_test_question_with_choices(QUESDROP, '\\mod_kilman\\question\\drop', array('content' => 'Select one'));
    }
    ...
    public function test_create_question_essay() {
        $questiondata = array(
            ...
            'length' => 0,
            'precise' => 5);
        $this->create_test_question(QUESESSAY, '\\mod_kilman\\question\\essay', $questiondata);
    }

    public function test_create_question_sectiontext() {
        $this->create_test_question(QUESSECTIONTEXT, '\\mod_kilman\\question\\sectiontext',
    ...
    public function test_create_question_numeric() {
        $questiondata = array(
            ...
            'length' => 10,
            'precise' => 0);
        $this->create_test_question(QUESNUMERIC, '\\mod_kilman\\question\\numerical', $questiondata);
    }

    public function test_create_question_radiobuttons() {
        $this->create_test_question_with_choices(QUESRADIO,
            '\\mod_kilman\\question\\radio', array('content' => 'Choose one'));
    }

    public function test_create_question_ratescale() {
        $this->create_test_question_with_choices(QUESRATE, '\\mod_kilman\\question\\rate', array('content' => 'Rate these'));
    }
    ...
    public function test_create_question_textbox() {
        $questiondata = array(
            ...
            'length' => 20,
            'precise' => 25);
        $this->create_test_question(QUESTEXT, '\\mod_kilman\\question\\text', $questiondata);
    }

    public function test_create_question_yesno() {
        $this->create_test_question(QUESYESNO, '\\mod_kilman\\question\\yesno', array('content' => 'Enter yes or no'));
    }
    ...
    private function create_test_question($qtype, $questionclass, $questiondata = array(), $choicedata = null) {
        ...
        $question = $generator->create_question($kilman, $questiondata, $choicedata);
        ...
        if ($question->has_choices()) {
            ...
            foreach ($question->choices as $cid => $choice) {
                $this->assertTrue($DB->record_exists('kilman_quest_choice', array('id' => $cid)));
        ...
        // kilman object should now have question record(s).
        $kilman = new kilman($kilman->id, null, $course, $cm, true);
        $this->assertTrue($DB->record_exists('kilman_question', array('id' => $question->id)));
        $this->assertEquals('array', gettype($kilman->questions));
        $this->assertTrue(array_key_exists($question->id, $kilman->questions));
        $this->assertEquals(1, count($kilman->questions));
        if ($kilman->questions[$question->id]->has_choices()) {
            $this->assertEquals(count($choicedata), count($kilman->questions[$question->id]->choices));
        }
    }
