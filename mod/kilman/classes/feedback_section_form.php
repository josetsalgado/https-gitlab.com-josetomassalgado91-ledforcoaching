...
class feedback_section_form extends \moodleform {
    ...
    public function definition() {
        ...
        $feedbacksections = $kilman->survey->feedbacksections;
        $this->_feedbacks = $feedbacksection->sectionfeedback;
        $this->context = $kilman->context;
        $mform    =& $this->_form;

        if ($survey->feedbacksections > 1) {
            $mform->addElement('header', 'feedbacksectionsheader', get_string('feedbacksections', 'kilman'));
            $sselectgroup = [];
            $sselect = $mform->createElement('select', 'navigatesections', get_string('navigatetosection', 'kilman'),
                $this->_customdata->sectionselect);
            $sselect->setSelected($feedbacksection->id);
            $sselectgroup[] = $sselect;
            $sselectgroup[] = $mform->createElement('submit', 'gotosection', get_string('navigatetosection', 'kilman'));
            $mform->addGroup($sselectgroup, '');
            $addnewsectionarray = [];
            $addnewsectionarray[] = $mform->createElement('text', 'newsectionlabel',
                get_string('feedbacksectionlabel', 'kilman'));
            $mform->setType('newsectionlabel', PARAM_TEXT);
            $addnewsectionarray[] = $mform->createElement('submit', 'addnewsection', get_string('addnewsection', 'kilman'));
            $mform->addGroup($addnewsectionarray, '', get_string('feedbacksectionlabel', 'kilman'));
        }

        if ($feedbacksections == 1) {
            $label = get_string('feedbackglobal', 'kilman');
            $feedbackheading = get_string('feedbackglobalheading', 'kilman');
            $feedbackmessages = get_string('feedbackglobalmessages', 'kilman');
        } else {
            $label = $feedbacksection->sectionlabel;
            $feedbackheading = get_string('feedbacksectionheading', 'kilman', $label);
            $feedbackmessages = get_string('feedbackmessages', 'kilman', $label);
        }

        $mform->addElement('header', 'contenthdr', $feedbackheading);
        $mform->addElement('text', 'sectionlabel', get_string('feedbacksectionlabel', 'kilman'),
            ['size' => '50', 'maxlength' => '50']);
        $mform->setType('sectionlabel', PARAM_TEXT);
        $mform->addRule('sectionlabel', null, 'required', null, 'client');
        $mform->addHelpButton('sectionlabel', 'feedbacksectionlabel', 'kilman');

        $editoroptions = ['maxfiles' => EDITOR_UNLIMITED_FILES, 'trusttext' => true];
        $mform->addElement('editor', 'sectionheading', get_string('feedbacksectionheadingtext', 'kilman'),
            null, $editoroptions);
        $mform->setType('sectionheading', PARAM_RAW);
        $mform->setDefault('feedbacknotes', $kilman->survey->feedbacknotes);
        $mform->addHelpButton('sectionheading', 'feedbackheading', 'kilman');

        if ($kilman->survey->feedbacksections > 0) {
            // Sections.
            if ($survey->feedbacksections > 1) {
                $mform->addElement('header', 'fbsection_' . $feedbacksection->id,
                    get_string('feedbacksectionquestions', 'kilman', $label));
                $qvalid = $validquestions;
                if (!empty($feedbacksection->scorecalculation)) {
                    $rsrc = $kilman->renderer->image_url('t/delete');
                    $strremove = get_string('remove', 'kilman');
                    ...
                    foreach ($feedbacksection->scorecalculation as $qid => $score) {
                        ...
                        $mform->addGroup($questionactions, '', $kilman->questions[$qid]->name);
                        $counter++;
                    }
                }
                if (!empty($qvalid)) {
                    ...
                    $qselect[] = $mform->createElement('select', 'addquestionselect',
                        get_string('addquestiontosection', 'kilman'), $qvalid);
                    $qselect[] = $mform->createElement('submit', 'addquestion', get_string('addquestion', 'kilman'));
                    $mform->addGroup($qselect, '', get_string('addquestiontosection', 'kilman'));
                }
            }
        }
        ...
        $mform->addHelpButton('feedbackhdr', 'feedback', 'kilman');

        $mform->addElement('static', 'scoreboundarystatic1', get_string('feedbackscoreboundary', 'kilman'), '100%');

        $repeatarray = [];
        $repeatedoptions = [];

        $repeatarray[] = $mform->createElement('editor', 'feedbacktext', get_string('feedback', 'kilman'), null,
            ['maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $kilman->context]);
        $repeatarray[] = $mform->createElement(
            'text', 'feedbackboundaries', get_string('feedbackscoreboundary', 'kilman'), ['size' => 10]);
        ...
        $nextel = $this->repeat_elements($repeatarray, $numfeedbacks - 1, $repeatedoptions, 'boundary_repeats',
            'boundary_add_fields', 2, get_string('feedbackaddmorefeedbacks', 'kilman'), true);

        // Put some extra elements in before the button.
        $mform->insertElementBefore(
            $mform->createElement('editor', "feedbacktext[$nextel]", get_string('feedback', 'kilman'), null,
                ['maxfiles' => EDITOR_UNLIMITED_FILES, 'noclean' => true, 'context' => $kilman->context]),
            'boundary_add_fields');
        $mform->insertElementBefore(
            $mform->createElement('static', 'scoreboundarystatic2', get_string('feedbackscoreboundary', 'kilman'), '0%'),
            'boundary_add_fields');
        ...
        $buttonarray[] = $mform->createElement('submit', 'confirmdeletesection', get_string('deletesection', 'kilman'));
    ...
    public function validation($data, $files) {
        ...
        while (!empty($data['feedbackboundaries'][$i])) {
            ...
            if (is_numeric($boundary) && $boundary <= 0) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorboundaryoutofrange', 'kilman', $i + 1);
            }
            if (is_numeric($boundary) && $i > 0 &&
                    $boundary >= $data['feedbackboundaries'][$i - 1]) {
                $errors["feedbackboundaries[$i]"] = get_string('feedbackerrororder', 'kilman', $i + 1);
            }
        ...
        if (!empty($data['feedbackboundaries'])) {
            for ($i = $numboundaries; $i < count($data['feedbackboundaries']); $i += 1) {
                if (!empty($data['feedbackboundaries'][$i] ) &&
                    ...
                    $errors["feedbackboundaries[$i]"] = get_string('feedbackerrorjunkinboundary', 'kilman', $i + 1);
                }
            }
        }
        for ($i = $numboundaries + 1; $i < count($data['feedbacktext']); $i += 1) {
            if (!empty($data['feedbacktext'][$i]['text']) &&
                ...
                $errors["feedbacktext[$i]"] = get_string('feedbackerrorjunkinfeedback', 'kilman', $i + 1);
            }
        }
        return $errors;
    }
