...
/**
 * Contains class mod_kilman\search\question
 *
 * @package    mod_kilman
 ...
 */
namespace mod_kilman\search;
defined('MOODLE_INTERNAL') || die();
/**
 * Search area for mod_kilman questions. Separated from the activity search so that admins can choose whether or not they
...
class question extends \core_search\base_mod {
    ...
    public function get_recordset_by_timestamp($modifiedfrom = 0) {
        ...
        // Join the survey record to ensure only kilmans with questions are returned.
        $sql = 'SELECT q.* ' .
            'FROM {kilman} q ' .
     ...
     * questions associated with a kilman.
     *
    ...
    public function get_document($record, $options = []) {
        ...
        try {
            $cm = $this->get_cm('kilman', $record->id, $record->course);
        ...
        $recordset = $DB->get_recordset('kilman_question', ['surveyid' => $record->sid, 'deleted' => 'n'],
            'id', 'id,content');
    ...
    public function check_access($id) {
        global $DB;
        try {
            $activity = $DB->get_record('kilman', ['id' => $id], '*', MUST_EXIST);
            $cminfo = $this->get_cm('kilman', $activity->id, $activity->course);
        ...
        // If the user has the ability to see questions beyond completing a kilman, grant access.
        $context = \context_module::instance($cminfo->id);
        if (!(has_capability('mod/kilman:readallresponses', $context) ||
              has_capability('mod/kilman:readallresponseanytime', $context) ||
              has_capability('mod/kilman:editquestions', $context))) {
            return \core_search\manager::ACCESS_DENIED;
        }
    ...
    public function get_context_url(\core_search\document $doc) {
        ...
        return new \moodle_url('/mod/kilman/view.php', ['id' => $context->instanceid]);
    }
}