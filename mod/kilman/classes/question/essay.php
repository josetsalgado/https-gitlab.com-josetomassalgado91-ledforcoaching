...
class essay extends base {
    ...
    protected function responseclass() {
        return '\\mod_kilman\\response\\text';
    }
    ...
    protected function question_survey_display($data, $descendantsdata, $blankkilman=false) {
        ...
        if ($canusehtmleditor) {
            ...
            $editor->use_editor($name, kilman_get_editor_options($this->context));
    ...
    protected function form_length(\MoodleQuickForm $mform, $helptext = '') {
        $responseformats = array(
                        "0" => get_string('formateditor', 'kilman'),
                        "1" => get_string('formatplain', 'kilman'));
        $mform->addElement('select', 'precise', get_string('responseformat', 'kilman'), $responseformats);
    ...
    protected function form_precise(\MoodleQuickForm $mform, $helptext = '') {
        ...
        for ($lines = 5; $lines <= 40; $lines += 5) {
            $choices[$lines] = get_string('nlines', 'kilman', $lines);
        }
        $mform->addElement('select', 'length', get_string('responsefieldlines', 'kilman'), $choices);
