...
namespace mod_kilman\question;
use mod_kilman\edit_question_form;
use \kilman;
...
/**
 * This file contains the parent class for kilman question types.
 *
...
abstract class base {
    ...
    /** @var int $position Position in the kilman */
    public $position    = 0;
    ...
    public function __construct($id = 0, $question = null, $context = null, $params = []) {
        ...
        if ($qtypes === null) {
            $qtypes = $DB->get_records('kilman_question_type', [], 'typeid',
                                       'typeid, type, has_choices, response_table');
        }

        if ($id) {
            $question = $DB->get_record('kilman_question', ['id' => $id]);
        }
    ...
    static public function question_builder($qtype, $qdata = null, $context = null) {
        $qclassname = '\\mod_kilman\\question\\'.self::qtypename($qtype);
        $qid = 0;
    ...
    private function get_choices() {
        ...
        if ($choices = $DB->get_records('kilman_quest_choice', ['question_id' => $this->id], 'id ASC')) {
    ...
    private function get_dependencies() {
        ...
        $this->dependencies = [];
        $dependencies = $DB->get_records('kilman_dependency',
    ...
    protected function get_dependency_options() {
        ...
        if ($this->allows_dependents() && $this->has_choices()) {
            foreach ($this->choices as $key => $choice) {
                $contents = kilman_choice_values($choice->content);
    ...
    public function insert_response($rid, $val) {
        if (isset ($this->response) && is_object($this->response) &&
            is_subclass_of($this->response, '\\mod_kilman\\response\\base')) {
     ...
     *
     * @return object The response object based off of kilman_response_base.
     *
     */
    ...
    public function update($questionrecord = null, $updatechoices = true) {
        ...
        $DB->update_record('kilman_question', $questionrecord);
    ...
    public function add($questionrecord, array $choicerecords = null, $calcposition = true) {
        ...
        $this->qid = $DB->insert_record('kilman_question', $questionrecord);
    ...
    public function update_choice($choicerecord) {
        global $DB;
        return $DB->update_record('kilman_quest_choice', $choicerecord);
    }

    public function add_choice($choicerecord) {
        global $DB;
        $retvalue = true;
        if ($cid = $DB->insert_record('kilman_quest_choice', $choicerecord)) {
    ...
    public function delete_choice($choice) {
        ...
        if ($DB->delete_records('kilman_quest_choice', ['id' => $cid])) {
    ...
    public function update_dependency($dependencyrecord) {
        global $DB;
        return $DB->update_record('kilman_dependency', $dependencyrecord);
    }
    ...
    public function add_dependency($dependencyrecord) {
        ...
        $retvalue = true;
        if ($did = $DB->insert_record('kilman_dependency', $dependencyrecord)) {
    ...
    public function delete_dependency($dependency) {
        ...
        if ($DB->delete_records('kilman_dependency', ['id' => $did])) {
    ...
    public function set_required($required) {
        ...
        $this->required = $rval;
        return $DB->set_field('kilman_question', 'required', $rval, ['id' => $qid]);
    }
     ...
     *
     */
    abstract protected function question_survey_display($formdata, $descendantsdata, $blankkilman);
     ...
     * @param boolean $blankkilman
     */
    public function question_output($formdata, $dependants=[], $qnum='', $blankkilman) {
        $pagetags = $this->questionstart_survey_display($qnum, $formdata);
        $pagetags->qformelement = $this->question_survey_display($formdata, $dependants, $blankkilman);
        return $pagetags;
    }
...
//        $pagetags->qformelement = $this->response_survey_display($data);
    ...
    public function questionstart_survey_display($qnum, $formdata='') {
        global $OUTPUT, $SESSION, $kilman, $PAGE;
        ...
        // If we are on report page and this kilman has dependquestions and this question was skipped.
        if (($pagetype == 'mod-kilman-myreport' || $pagetype == 'mod-kilman-report') &&
        ...
        // In report mode, If kilman is set to no numbering,
        // also hide answers to questions that have not been answered.
        $displayclass = 'qn-container';
        if ($pagetype == 'mod-kilman-preview' || ($nonumbering &&
            ($currenttab == 'mybyresponse' || $currenttab == 'individualresp'))) {
            // This needs to be done to ensure all dependency data is loaded.
            // TODO - Perhaps this should be a function called by the kilman after it loads all questions?
            $kilman->load_parents($this);
            // Want this to come from the renderer, meaning we need $kilman.
            $pagetags->dependencylist = $kilman->renderer->get_dependency_html($this->id, $this->dependencies);
        }
        ...
        if ($this->type_id != QUESSECTIONTEXT) {
            ...
            if ($this->required()) {
                ...
                $required .= get_string('required', 'kilman');
                $required .= html_writer::end_tag('div');
                $required .= html_writer::empty_tag('img', ['class' => 'req', 'title' => get_string('required', 'kilman'),
                    'alt' => get_string('required', 'kilman'), 'src' => $OUTPUT->image_url('req')]);
            }
        ...
        $content = format_text(file_rewrite_pluginfile_urls($this->content, 'pluginfile.php',
            $this->context->id, 'mod_kilman', 'question', $this->id), FORMAT_HTML, $options);
     ...
     * @param kilman $kilman The kilman being edited.
     * @return bool
     */
    public function edit_form(edit_question_form $form, kilman $kilman) {
        $mform =& $form->_form;
        ...
        if ($kilman->navigate > 0) {
            $this->form_dependencies($form, $kilman->questions);
        }
        ...
        if (isset($this->qid)) {
            $buttonarray[] = &$mform->createElement('submit', 'makecopy', get_string('saveasnew', 'kilman'));
        }
    ...
    protected function form_header(\MoodleQuickForm $mform, $helpname = '') {
        ...
        if (isset($this->qid) && !empty($this->qid)) {
            $header = get_string('editquestion', 'kilman', kilman_get_type($this->type_id));
        } else {
            $header = get_string('addnewquestion', 'kilman', kilman_get_type($this->type_id));
        }
        if (empty($helpname)) {
        ...
        $mform->addHelpButton('questionhdredit', $helpname, 'kilman');
    }

    protected function form_name(\MoodleQuickForm $mform) {
        $mform->addElement('text', 'name', get_string('optionalname', 'kilman'),
                        ['size' => '30', 'maxlength' => '30']);
        $mform->setType('name', PARAM_TEXT);
        $mform->addHelpButton('name', 'optionalname', 'kilman');
        return $mform;
    }
    ...
    protected function form_required(\MoodleQuickForm $mform) {
        ...
        $mform->addGroup($reqgroup, 'reqgroup', get_string('required', 'kilman'), ' ', false);
        $mform->addHelpButton('reqgroup', 'required', 'kilman');
        return $mform;
    }
    ...
    protected function form_dependencies($form, $questions) {
        ...
        if (count($dependencies) > 1) {
            $mform->addElement('header', 'dependencies_hdr', get_string('dependencies', 'kilman'));
             ...
             * bottom up, starting at the lowest child question is a pain for large kilmans.
            ...
            $mform->addElement('static', 'mandatory', '',
                '<div class="dimmed_text">' . get_string('mandatory', 'kilman') . '</div>');
            $selectand = $mform->createElement('select', 'dependlogic_and', get_string('condition', 'kilman'),
                [get_string('answernotgiven', 'kilman'), get_string('answergiven', 'kilman')]);
            $selectand->setSelected('1');
            $groupitemsand = [];
            $groupitemsand[] =& $mform->createElement('selectgroups', 'dependquestions_and',
                get_string('parent', 'kilman'), $dependencies);
            $groupitemsand[] =& $selectand;
            $groupand = $mform->createElement('group', 'selectdependencies_and', get_string('dependquestion', 'kilman'),
                $groupitemsand, ' ', false);
            ...
            $mform->addElement('static', 'optional', '',
                '<div class="dimmed_text">' . get_string('optional', 'kilman') . '</div>');
            $selector = $mform->createElement('select', 'dependlogic_or', get_string('condition', 'kilman'),
                [get_string('answernotgiven', 'kilman'), get_string('answergiven', 'kilman')]);
            ...
            $groupor = $mform->createElement('group', 'selectdependencies_or', get_string('dependquestion', 'kilman'),
                $groupitemsor, ' ', false);
    ...
    protected function form_question_text(\MoodleQuickForm $mform, $context) {
        ...
        $mform->addElement('editor', 'content', get_string('text', 'kilman'), null, $editoroptions);
    ...
    protected function form_choices(\MoodleQuickForm $mform, array $choices, $helpname = '') {
        ...
        $mform->addElement('textarea', 'allchoices', get_string('possibleanswers', 'kilman'), $options);
        $mform->setType('allchoices', PARAM_RAW);
        $mform->addRule('allchoices', null, 'required', null, 'client');
        $mform->addHelpButton('allchoices', $helpname, 'kilman');
    ...
    static public function form_length_text(\MoodleQuickForm $mform, $helpname = '', $value = 0) {
        $mform->addElement('text', 'length', get_string($helpname, 'kilman'), ['size' => '1'], $value);
        $mform->setType('length', PARAM_INT);
        if (!empty($helpname)) {
            $mform->addHelpButton('length', $helpname, 'kilman');
        }
        return $mform;
    }
    ...
    static public function form_precise_text(\MoodleQuickForm $mform, $helpname = '', $value = 0) {
        $mform->addElement('text', 'precise', get_string($helpname, 'kilman'), ['size' => '1']);
        $mform->setType('precise', PARAM_INT);
        if (!empty($helpname)) {
            $mform->addHelpButton('precise', $helpname, 'kilman');
        }
        return $mform;
    }
     ...
     */
    public function form_update($formdata, $kilman) {
        global $DB;
        ...
        if (!empty($formdata->qid)) {
            ...
            $formdata->content = file_save_draft_area_files($formdata->itemid, $kilman->context->id, 'mod_kilman',
            ...
            $formdata->format  = $formdata->content['format'];
            $formdata->content = $formdata->content['text'];
            $content           = file_save_draft_area_files($formdata->itemid, $kilman->context->id, 'mod_kilman',
                'question', $this->qid, ['subdirs' => true], $formdata->content);
            $DB->set_field('kilman_question', 'content', $content, ['id' => $this->qid]);
        }
    ...
    protected function form_preprocess_choicedata($formdata) {
        if (empty($formdata->allchoices)) {
            error (get_string('enterpossibleanswers', 'kilman'));
        }
        return false;
    }
}