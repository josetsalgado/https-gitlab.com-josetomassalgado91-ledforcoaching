...
class rate extends base {
    ...
    protected function responseclass() {
        return '\\mod_kilman\\response\\rank';
    }
    ...
    public function question_template() {
        return 'mod_kilman/question_rate';
    }
    ...
    public function response_template() {
        return 'mod_kilman/response_rate';
    }
    ...
    protected function question_survey_display($data, $descendantsdata, $blankkilman=false) {
        ...
        if ($isna) {
            $na = get_string('notapplicable', 'kilman');
        } else {
            $na = '';
        }
        if ($this->precise == 2) {
        ...
        if ($nbchoices > 1 && $this->precise != 2 && !$blankkilman) {
        ...
        if ( ($num != $nbchoices) && ($num != 0) ) {
            $this->add_notification(get_string('checkallradiobuttons', 'kilman', $nbchoices));
            $notcomplete = true;
        }
            ...
            if (isset($choice->content)) {
                ...
                if ($nbchoices > 1 && $this->precise != 2  && !$blankkilman) {
                    ...
                    if ($notcomplete && isset($data->$str) && ($data->$str == -999)) {
                        ...
                        $title = get_string('pleasecomplete', 'kilman');
                    }
                ...
                for ($j = 0; $j < $this->length + $isna; $j++) {
                    ...
                    $col['colclass'] = $bg;
                    $i = $j + 1;
                    $col['colhiddentext'] = get_string('option', 'kilman', $i);
    ...
    protected function response_survey_display($data) {
        ...
        if ($this->precise == 1) {
            $cellobj = new \stdClass();
            $cellobj->bg = $bg;
            $cellobj->str = get_string('notapplicable', 'kilman');
            $resptags->headers[] = $cellobj;
        }
    ...
    protected function form_precise(\MoodleQuickForm $mform, $helptext = '') {
        $precoptions = array("0" => get_string('normal', 'kilman'),
                             "1" => get_string('notapplicablecolumn', 'kilman'),
                             "2" => get_string('noduplicates', 'kilman'),
                             "3" => get_string('osgood', 'kilman'));
        $mform->addElement('select', 'precise', get_string('kindofratescale', 'kilman'), $precoptions);
        $mform->addHelpButton('precise', 'kindofratescale', 'kilman');
