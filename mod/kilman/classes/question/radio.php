...
class radio extends base {
    ...
    protected function responseclass() {
        return '\\mod_kilman\\response\\single';
    }
    ...
    public function question_template() {
        return 'mod_kilman/question_radio';
    }
    ...
    public function response_template() {
        return 'mod_kilman/response_radio';
    }
     ...
     *
     */
    protected function question_survey_display($data, $dependants=[], $blankkilman=false) {
        // Radio buttons
        ...
        foreach ($this->choices as $id => $choice) {
            ...
            if ($other !== 0) { // This is a normal radio button.
                ...
                $othertext = preg_replace(["/^!other=/", "/^!other/"], ['', get_string('other', 'kilman')],
                    $choice->content);
                $cid = 'q'.$this->id.'_'.$id;
                $otherempty = false;
        ...
        if (!$this->required()) {
            ...
            if (!$ischecked && !$blankkilman) {
                $radio->checked = true;
            }
            $content = get_string('noanswer', 'kilman');
        ...
        if ($otherempty) {
            $this->add_notification(get_string('otherempty', 'kilman'));
        }
        return $choicetags;
    }
     ...
     *
     */
    public function response_surveypdf_display($data) {
    ...
    protected function form_length(\MoodleQuickForm $mform, $helptext = '') {
        $lengroup = [];
        $lengroup[] =& $mform->createElement('radio', 'length', '', get_string('vertical', 'kilman'), '0');
        $lengroup[] =& $mform->createElement('radio', 'length', '', get_string('horizontal', 'kilman'), '1');
        $mform->addGroup($lengroup, 'lengroup', get_string('alignment', 'kilman'), ' ', false);
        $mform->addHelpButton('lengroup', 'alignment', 'kilman');
