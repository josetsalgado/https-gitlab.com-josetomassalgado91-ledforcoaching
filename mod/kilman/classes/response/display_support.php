...
class display_support {
    ...
    public static function mkresavg($counts, $total, $choices, $precision, $showtotals, $length, $sort, $stravgvalue='') {
        global $CFG;
        $stravgrank = get_string('averagerank', 'kilman');
        $osgood = false;
        if ($precision == 3) { // Osgood's semantic differential.
            $osgood = true;
            $stravgrank = get_string('averageposition', 'kilman');
        }
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'kilman');
        }
        ...
        if (!empty($counts) && is_array($counts)) {
            ...
            $table->data[] = array('', get_string('noresponsedata', 'kilman'));
        }
    ...
    public static function mkrescount($counts, $rids, $rows, $question, $precision, $length, $sort) {
        ...
        $strtotal = '<strong>'.get_string('total', 'kilman').'</strong>';
        $isna = $precision == 1;
        $isnahead = '';
        $osgood = false;
        if ($precision == 3) { // Osgood's semantic differential.
        ...
        if ($isna) {
            $isnahead = get_string('notapplicable', 'kilman').'<br />(#)';
        }
        if ($precision == 1) {
        ...
        $headings = array('<span class="smalltext">'.get_string('responses', 'kilman').'</span>');
        if ($osgood) {
            $align = array('right');
        } else {
            $align = array('left');
        ...
        if ($isrestricted) {
            array_push($headings, get_string('notapplicable', 'kilman'));
            array_push($align, 'center');
        }
        array_push($align, 'center');
        if ($na) {
