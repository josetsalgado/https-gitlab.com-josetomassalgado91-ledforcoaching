...
class single extends base {
            ...
            foreach ($this->question->choices as $cid => $choice) {
                if (strpos($choice->content, '!other') === 0) {
                    ...
                    if (preg_match("/[^ \t\n]/", $other)) {
                        ...
                        $resid = $DB->insert_record('kilman_response_other', $record);
                        $val = $cid;
                        break;
                    }
                }
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        $sql = 'SELECT rt.id, rt.response, qc.content ' .
               'FROM {kilman_response_other} rt, ' .
                    '{kilman_quest_choice} qc ' .
    ...
    public function results_template() {
        return 'mod_kilman/results_choice';
    }
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $sql = 'SELECT q.id '.$col.', q.type_id as q_type, c.content as ccontent,c.id as cid '.
            'FROM {'.static::response_table().'} a, {kilman_question} q, {kilman_quest_choice} c '.
        ...
        foreach ($records as $qid => $row) {
            ...
            if ($csvexport) {
                static $i = 1;
                $qrecords = $DB->get_records('kilman_quest_choice', ['question_id' => $qid]);
                foreach ($qrecords as $value) {
                    if ($value->id == $cid) {
                        $contents = kilman_choice_values($value->content);
                        if ($contents->modname) {
                            $row->ccontent = $contents->modname;
                        } else {
                            $content = $contents->text;
                            if (preg_match('/^!other/', $content)) {
                                $row->ccontent = get_string('other', 'kilman');
     ...
     * @author Guy Thomas
     * @param int|array $kilmanids One id, or an array of ids.
     * @param bool|int $responseid
     * @param bool|int $userid
     * @return array
     */
    public function get_bulk_sql($kilmanids, $responseid = false, $userid = false, $groupid = false, $showincompletes = 0) {
        global $DB;
        ...
        if (is_array($kilmanids)) {
            list($qsql, $params) = $DB->get_in_or_equal($kilmanids);
        } else {
            $qsql = ' = ? ';
            $params = [$kilmanids];
        }
        ...
        $sql .= "
            AND qr.kilmanid $qsql $showcompleteonly
      LEFT JOIN {kilman_response_other} qro ON qro.response_id = qr.id AND qro.choice_id = qrs.choice_id
