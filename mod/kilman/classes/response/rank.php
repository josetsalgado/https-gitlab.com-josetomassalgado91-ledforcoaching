...
namespace mod_kilman\response;
defined('MOODLE_INTERNAL') || die();

use mod_kilman\db\bulk_sql_config;
...
class rank extends base {
    ...
    public function insert_response($rid, $val) {
        ...
        if ($this->question->type_id == QUESRATE) {
            ...
            foreach ($this->question->choices as $cid => $choice) {
                ...
                if ($other == get_string('notapplicable', 'kilman')) {
                    $rank = -1;
                } else {
                    $rank = intval($other);
                }
            ...
            if ($val == get_string('notapplicable', 'kilman')) {
                $rank = -1;
            } else {
                $rank = intval($val);
            }
    ...
    public function get_results($rids=false, $anonymous=false) {
        ...
        if ($this->question->type_id == QUESRATE) {
            ...
            if ($rows = $DB->get_records_select('kilman_quest_choice', $select)) {
                foreach ($rows as $row) {
            ...
            if (!$isrestricted) {
                if (!empty ($rankvalue)) {
                    ...
                    FROM {kilman_quest_choice} c, {".self::response_table()."} r
                ...
                $sql = "SELECT c.id, c.content, a.average, a.num
                        FROM {kilman_quest_choice} c
                        INNER JOIN
                             (SELECT c2.id, AVG(a2.rankvalue+1) AS average, COUNT(a2.response_id) AS num
                              FROM {kilman_quest_choice} c2, {".self::response_table()."} a2
    ...
    public function display_results($rids=false, $sort='', $anonymous=false) {
        ...
        if ($rows = $this->get_results($rids, $sort, $anonymous)) {
                ...
                if (array_key_exists($ccontent, $rows)) {
                    ...
                    if (isset($rows[$ccontent]->averagevalue)) {
                        ...
                        if ($stravgvalue == '' && !$osgood) {
                            $stravgvalue = ' ('.get_string('andaveragevalues', 'kilman').')';
                        }
                    } else {
                        $avgvalue = null;
                    }
            ...
            $output .= \mod_kilman\response\display_support::mkresavg($this->counts, count($rids), $this->question->choices,
                $this->question->precise, $prtotal, $this->question->length, $sort, $stravgvalue);

            $output .= \mod_kilman\response\display_support::mkrescount($this->counts, $rids, $rows, $this->question,
                $this->question->precise, $this->question->length, $sort);
        } else {
            $output .= '<p class="generaltable">&nbsp;'.get_string('noresponsedata', 'kilman').'</p>';
        }
        return $output;
    }
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $sql = 'SELECT a.id as aid, q.id AS qid, q.precise AS precise, c.id AS cid '.$col.', c.content as ccontent,
            ...
            'FROM {'.self::response_table().'} a, {kilman_question} q, {kilman_quest_choice} c '.
                ...
                if ($key != 'content') { // No need to keep question text - ony keep choice text and rank.
                    if ($key == 'ccontent') {
                        if ($osgood) {
                            ...
                            $contents = kilman_choice_values($val);
                            if ($contents->modname) {
