...
class text extends base {
    ...
    public function results_template() {
        return 'mod_kilman/results_text';
    }
     ...
     *
     * @param $weights
     * @param $participants Number of kilman participants.
    ...
    public function get_results_tags($weights, $participants, $respondents, $showtotals = 1, $sort = '') {
        ...
        if (is_object(reset($weights))) {
            global $CFG, $SESSION, $kilman, $DB;
            $viewsingleresponse = $kilman->capabilities->viewsingleresponse;
            $nonanonymous = $kilman->respondenttype != 'anonymous';
            if ($viewsingleresponse && $nonanonymous) {
                $currentgroupid = '';
                if (isset($SESSION->kilman->currentgroupid)) {
                    $currentgroupid = $SESSION->kilman->currentgroupid;
                }
                $url = $CFG->wwwroot.'/mod/kilman/report.php?action=vresp&amp;sid='.$kilman->survey->id.
            ...
            $nbresponses = 0;
            $sum = 0;
            $strtotal = get_string('totalofnumbers', 'kilman');
            $straverage = get_string('average', 'kilman');
    ...
    static public function response_select($rid, $col = null, $csvexport = false, $choicecodes = 0, $choicetext = 1) {
        ...
        $sql = 'SELECT q.id '.$col.', a.response as aresponse '.
            'FROM {'.self::response_table().'} a, {kilman_question} q '.
