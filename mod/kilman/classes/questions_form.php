...
class questions_form extends \moodleform {
    ...
    public function definition() {
        global $CFG, $kilman, $SESSION;
        global $DB;

        $sid = $kilman->survey->id;
        $mform    =& $this->_form;

        $mform->addElement('header', 'questionhdr', get_string('addquestions', 'kilman'));
        $mform->addHelpButton('questionhdr', 'questiontypes', 'kilman');

        $strremove = get_string('remove', 'kilman');
        $strmove = get_string('move');
        $strmovehere = get_string('movehere');
        $strposition = get_string('position', 'kilman');

        if (!isset($kilman->questions)) {
            $kilman->questions = array();
        }
        if ($this->moveq) {
            $moveqposition = $kilman->questions[$this->moveq]->position;
        }

        $pos = 0;
        $select = '';
        if (!($qtypes = $DB->get_records_select_menu('kilman_question_type', $select, null, '', 'typeid,type'))) {
            $qtypes = array();
        }
        // Get the names of each question type in the appropriate language.
        foreach ($qtypes as $key => $qtype) {
            // Do not allow "Page Break" to be selected as first element of a kilman.
            if (empty($kilman->questions) && ($qtype == 'Page Break')) {
                unset($qtypes[$key]);
            } else {
                $qtypes[$key] = kilman_get_type($key);
            }
        }
        natsort($qtypes);
        $addqgroup = array();
        ...
        if (isset($SESSION->kilman->type_id)) {
                $mform->setDefault('type_id', $SESSION->kilman->type_id);
        }

        $addqgroup[] =& $mform->createElement('submit', 'addqbutton', get_string('addselqtype', 'kilman'));

        $kilmanhasdependencies = $kilman->has_dependencies();

        $mform->addGroup($addqgroup, 'addqgroup', '', ' ', false);

        if (isset($SESSION->kilman->validateresults) &&  $SESSION->kilman->validateresults != '') {
            $mform->addElement('static', 'validateresult', '', '<div class="qdepend warning">'.
                $SESSION->kilman->validateresults.'</div>');
            $SESSION->kilman->validateresults = '';
        }
        ...
        if ($kilmanhasdependencies) {
            $parentpositions = kilman_get_parent_positions($kilman->questions);
            $childpositions = kilman_get_child_positions($kilman->questions);
        }

        $mform->addElement('header', 'manageq', get_string('managequestions', 'kilman'));
        $mform->addHelpButton('manageq', 'managequestions', 'kilman');
        ...
        foreach ($kilman->questions as $question) {
            ...
            if ($kilmanhasdependencies) {
                ...
                $kilman->load_parents($question);
                $dependencies = $kilman->renderer->get_dependency_html($question->id, $question->dependencies);
            } else {
                $dependencies = '';
            }
            ...
            if ($tid != QUESPAGEBREAK) {
                ...
                $content = format_text(file_rewrite_pluginfile_urls($question->content, 'pluginfile.php',
                    $question->context->id, 'mod_kilman', 'question', $question->id), FORMAT_HTML, ['noclean' => true]);
            }
            $moveqgroup = array();

            $spacer = $kilman->renderer->image_url('spacer');
            ...
            if (!$this->moveq) {
                ...
                $eextra = array('value' => $question->id,
                                'alt' => get_string('edit', 'kilman'),
                                'title' => get_string('edit', 'kilman'));
                ...
                if ($tid == QUESPAGEBREAK) {
                    ...
                    $esrc = $kilman->renderer->image_url('t/edit');
                }
                ...
                $msrc = $kilman->renderer->image_url('t/move');
                ...
                if ($kilmanhasdependencies) {
                    ...
                    if ($pos == 1) {
                        if (isset($childpositions[$qid])) {
                            ...
                            if ($maxdown < 4) {
                                $strdisabled = get_string('movedisabled', 'kilman');
                                $msrc = $kilman->renderer->image_url('t/block');
                                $mextra = array('value' => $question->id,
                                                'alt' => $strdisabled,
                                                'title' => $strdisabled);
                    ...
                    if ($tid == QUESPAGEBREAK) {
                        if ($nextquestion = $DB->get_record('kilman_question',
                            ['surveyid' => $sid, 'position' => $pos + 1, 'deleted' => 'n'], 'id, name, content') ) {

                            $nextquestiondependencies = $DB->get_records('kilman_dependency',
                                ['questionid' => $nextquestion->id , 'surveyid' => $sid], 'id ASC');

                            if ($previousquestion = $DB->get_record('kilman_question',
                                ['surveyid' => $sid, 'position' => $pos - 1, 'deleted' => 'n'], 'id, name, content')) {

                                $previousquestiondependencies = $DB->get_records('kilman_dependency',
                                ...
                                if (!empty($nextquestiondependencies) ||
                                    ...
                                    $rsrc = $msrc;
                                    $strdisabled = get_string('deletedisabled', 'kilman');
                                    $rextra = array('value' => $question->id,
                                                    'alt' => $strdisabled,
                                                    'title' => $strdisabled);
                ...
                if ($tid != QUESPAGEBREAK && $tid != QUESSECTIONTEXT) {
                    if ($required == 'y') {
                        $reqsrc = $kilman->renderer->image_url('t/stop');
                        $strrequired = get_string('required', 'kilman');
                    } else {
                        $reqsrc = $kilman->renderer->image_url('t/go');
                        $strrequired = get_string('notrequired', 'kilman');
                    }
                    $strrequired .= ' '.get_string('clicktoswitch', 'kilman');
                ...
                $typeid = $DB->get_field('kilman_question', 'type_id', array('id' => $this->moveq));
                ...
                if ($display) {
                    ...
                    if ($typeid == QUESPAGEBREAK && $pos == 1) {
                        ...
                        if ($this->moveq == $question->id) {
                            ...
                            $msrc = $kilman->renderer->image_url('movehere');
