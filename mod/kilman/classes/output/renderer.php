...
/**
 * Contains class mod_kilman\output\renderer
 *
 * @package    mod_kilman
...
class renderer extends \plugin_renderer_base {
    ...
    public function render_viewpage($page) {
        ...
        return $this->render_from_template('mod_kilman/viewpage', $data);
    }

    /**
     * Fill out the kilman (complete) page.
     * @param \templateable $page
     * @return string | boolean
     */
    public function render_completepage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/completepage', $data);
    }
    ...
    public function render_reportpage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/reportpage', $data);
    }
    ...
    public function render_qsettingspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/qsettingspage', $data);
    }
    ...
    public function render_questionspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/questionspage', $data);
    }
    ...
    public function render_previewpage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/previewpage', $data);
    }
    ...
    public function render_nonrespondentspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/nonrespondentspage', $data);
    }
    ...
    public function render_fbsectionspage($page) {
        $data = $page->export_for_template($this);
        return $this->render_from_template('mod_kilman/fbsectionspage', $data);
    }
     ...
     * @param boolean $blankkilman Used for printing a blank one.
     * @return string The output for the page.
     */
    public function question_output($question, $formdata, $dependants=[], $qnum, $blankkilman) {

        $pagetags = $question->question_output($formdata, $dependants, $qnum, $blankkilman);
    ...
    public function all_response_output($data=null) {
        ...
        if (is_string($data)) {
            ...
            foreach ($data as $qnum => $responses) {
                ...
                $output .= $this->render_from_template('mod_kilman/response_container', $pagetags);
            }
        }
        return $output;
    }
    ...
    public function navigationbar($navbar) {
        return $this->render_from_template('mod_kilman/navbaralpha', $navbar);
    }
    ...
    public function usernavigationbar($navbar) {
        return $this->render_from_template('mod_kilman/navbaruser', $navbar);
    }
    ...
    public function responselist($navbar) {
        return $this->render_from_template('mod_kilman/responselist', $navbar);
    }
    ...
    public function dependency_warnings($children, $langstring, $strnum) {
        $msg = '<div class="warning">' . get_string($langstring, 'kilman') . '</div><br />';
        foreach ($children as $child) {
            ...
            foreach ($child as $subchild) {
                ...
                switch ($subchild->dependlogic) {
                    case 0:
                        $logic = get_string('notset', 'kilman');
                        break;
                    case 1:
                        $logic = get_string('set', 'kilman');
                        break;
                    default:
                        $logic = '';
                }
                ...
                if (!in_array($subchild->qdependquestion, $loopindicator)) {
                    $msg .= '<div class = "qn-container">'.$strnum.' '.$subchild->position.$childname.
                        ...
                        get_string('dependquestion', 'kilman').'</strong>'.
        ...
        foreach ($dependencies as $dependency) {
            ...
            if ($dependency->dependandor == "and") {
                $html .= '<div id="qdepend_' . $qid . '_' . $dependency->dependquestionid . '_' .
                    ...
                    get_string('dependquestion', 'kilman') . '</strong> : '. get_string('position', 'kilman') . ' ' .
