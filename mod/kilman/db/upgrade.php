...
function xmldb_kilman_upgrade($oldversion=0) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2007120101) {
        $result &= kilman_upgrade_2007120101();

        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2007120101, 'kilman');
    }

    if ($oldversion < 2007120102) {
        // Change enum values to lower case for all tables using them.
        $table = new xmldb_table('kilman_question');
        ...
        $DB->set_field('kilman_question', 'required', 'y', array('required' => 'Y'));
        $DB->set_field('kilman_question', 'required', 'n', array('required' => 'N'));
        ...
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, false, null, 'n');
        $dbman->change_field_enum($table, $field);
        $DB->set_field('kilman_question', 'deleted', 'y', array('deleted' => 'Y'));
        $DB->set_field('kilman_question', 'deleted', 'n', array('deleted' => 'N'));
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, XMLDB_ENUM, array('y', 'n'), 'n');
        $dbman->change_field_enum($table, $field);
        $dbman->change_field_default($table, $field);
        unset($field);
        ...
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, false, null, 'n');
        $dbman->change_field_enum($table, $field);
        $DB->set_field('kilman_question', 'public', 'y', array('public' => 'Y'));
        $DB->set_field('kilman_question', 'public', 'n', array('public' => 'N'));
        ...
        $table = new xmldb_table('kilman_question_type');

        $field = new xmldb_field('has_choices');
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, false, null, 'n');
        $dbman->change_field_enum($table, $field);
        $DB->set_field('kilman_question_type', 'has_choices', 'y', array('has_choices' => 'Y'));
        $DB->set_field('kilman_question_type', 'has_choices', 'n', array('has_choices' => 'N'));
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, XMLDB_ENUM, array('y', 'n'), 'y');
        $dbman->change_field_enum($table, $field);
        $dbman->change_field_default($table, $field);
        unset($field);
        ...
        $table = new xmldb_table('kilman_response');

        $field = new xmldb_field('complete');
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, false, null, 'n');
        $dbman->change_field_enum($table, $field);
        $DB->set_field('kilman_response', 'complete', 'y', array('complete' => 'Y'));
        $DB->set_field('kilman_response', 'complete', 'n', array('complete' => 'N'));
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, XMLDB_ENUM, array('y', 'n'), 'n');
        $dbman->change_field_enum($table, $field);
        $dbman->change_field_default($table, $field);
        unset($field);
        ...
        $table = new xmldb_table('kilman_response_bool');

        $field = new xmldb_field('choice_id');
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, false, null, 'n');
        $dbman->change_field_enum($table, $field);
        $DB->set_field('kilman_response_bool', 'choice_id', 'y', array('choice_id' => 'Y'));
        $DB->set_field('kilman_response_bool', 'choice_id', 'n', array('choice_id' => 'N'));
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, XMLDB_ENUM, array('y', 'n'), 'y');
        $dbman->change_field_enum($table, $field);
        $dbman->change_field_default($table, $field);
        unset($field);
        ...
        $table = new xmldb_table('kilman_survey');

        $field = new xmldb_field('public');
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, false, null, 'n');
        $dbman->change_field_enum($table, $field);
        $DB->set_field('kilman_survey', 'public', 'y', array('public' => 'Y'));
        $DB->set_field('kilman_survey', 'public', 'n', array('public' => 'N'));
        $field->set_attributes(XMLDB_TYPE_CHAR, '1', XMLDB_UNSIGNED, XMLDB_NOTNULL, null, XMLDB_ENUM, array('y', 'n'), 'y');
        $dbman->change_field_enum($table, $field);
        $dbman->change_field_default($table, $field);
        unset($field);
        ...
        $DB->set_field('kilman_question_type', 'response_table', 'resp_single',
                        array('response_table' => 'response_single'));
        $DB->set_field('kilman_question_type', 'response_table', 'resp_multiple',
                        array('response_table' => 'response_multiple'));

        // kilman savepoint reached..
        upgrade_mod_savepoint(true, 2007120102, 'kilman');
    }

    if ($oldversion < 2008031902) {
        $table = new xmldb_table('kilman');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008031902, 'kilman');
    }
    ...
    if ($oldversion < 2008031904) {
        ...
        if ($rs = $DB->get_recordset_sql($sql)) {
            ...
            $capview = 'mod/kilman:view';
            $capsubmit = 'mod/kilman:submit';

            foreach ($rs as $kilman) {
                $context = get_context_instance(CONTEXT_MODULE, $kilman->cmid);

                // Convert kilmans with resp_eligible = 'all' so that students & teachers have view and submit.
                if ($kilman->resp_eligible == 'all') {
                    ...
                    // Convert kilmans with resp_eligible = 'students' so that just students have view and submit.
                } else if ($kilman->resp_eligible == 'teachers') {
                    assign_capability($capsubmit, CAP_ALLOW, $editteacherroleid, $context->id, true);
                    assign_capability($capsubmit, CAP_ALLOW, $teacherroleid, $context->id, true);
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008031904, 'kilman');
    }
    ...
    if ($oldversion < 2008031905) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008031905, 'kilman');
    }

    if ($oldversion < 2008031906) {
        $table = new xmldb_table('kilman_response_rank');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008031906, 'kilman');
    }
    ...
    if ($oldversion < 2008060401) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008060401, 'kilman');
    }
    ...
    if ($oldversion < 2008070702) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008070702, 'kilman');
    }

    if ($oldversion < 2008070703) {
        $table = new xmldb_table('kilman_resp_multiple');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008070703, 'kilman');
    }
    ...
    if ($oldversion < 2008070704) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008070704, 'kilman');
    }
    ...
    if ($oldversion < 2008070705) {
        ...
        $DB->set_field('kilman', 'introformat', 1);

        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008070705, 'kilman');
    }
    ...
    if ($oldversion < 2008070706) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2008070706, 'kilman');
    }

    if ($oldversion < 2010110100) {
        // Drop list of values (enum) from field has_choices on table kilman_question_type.
        $table = new xmldb_table('kilman_question_type');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2010110100, 'kilman');
    }

    if ($oldversion < 2010110101) {
        // Drop list of values (enum) from field respondenttype on table kilman.
        $table = new xmldb_table('kilman');
        ...
        $dbman->drop_enum_from_field($table, $field);
        // Drop list of values (enum) from field resp_eligible on table kilman.
        ...
        // Drop list of values (enum) from field required on table kilman_question.
        $table = new xmldb_table('kilman_question');
        ...
        $dbman->drop_enum_from_field($table, $field);
        // Drop list of values (enum) from field deleted on table kilman_question.
        ...
        // Drop list of values (enum) from field complete on table kilman_response.
        $table = new xmldb_table('kilman_response');
        ...
        // Drop list of values (enum) from field choice_id on table kilman_response_bool.
        $table = new xmldb_table('kilman_response_bool');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2010110101, 'kilman');
    }

    if ($oldversion < 2012100800) {
        // Changing precision of field name on table kilman_survey to (255).
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2012100800, 'kilman');
    }

    if ($oldversion < 2013062302) {
        // Adding completionsubmit field to table kilman.
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2013062302, 'kilman');
    }

    if ($oldversion < 2013062501) {
        // Skip logic new feature.
        // Define field dependquestion to be added to kilman_question table.
        $table = new xmldb_table('kilman_question');
        ...
        $choices = $DB->get_recordset('kilman_quest_choice', null);
        $total = $DB->count_records('kilman_quest_choice');
        if ($total > 0) {
            ...
            foreach ($choices as $choice) {
                if (($choice->value == null || $choice->value == 'NULL')
                    ...
                    $content = kilman_choice_values($choice->content);
                    if (strpos($content->text, '=')) {
                        $newcontent = str_replace('=', '::', $content->text);
                        $choice->content = $newcontent;
                        $DB->update_record('kilman_quest_choice', $choice);
                    }
                }
                $pbar->update($i, $total, "Convert kilman choice value separator - $i/$total.");
                $i++;
            }
        }

        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2013062501, 'kilman');
    }
    ...
    if ($oldversion < 2013100500) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2013100500, 'kilman');
    }
    ...
    if ($oldversion < 2013122202) {
        ...
        // Define table kilman_fb_sections to be created.
        $table = new xmldb_table('kilman_fb_sections');
        ...
        // Adding keys to table kilman_fb_sections.
        ...
        // Define table kilman_feedback to be created.
        $table = new xmldb_table('kilman_feedback');
        $table->add_field('id', XMLDB_TYPE_INTEGER, '18', null, XMLDB_NOTNULL, XMLDB_SEQUENCE, null);
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2013122202, 'kilman');
    }
    ...
    if ($oldversion < 2014010300) {
        ...
        // kilman savepoint reached.
         upgrade_mod_savepoint(true, 2014010300, 'kilman');
    }

    if ($oldversion < 2015051101) {
        // Move the global config value for 'usergraph' to the plugin config setting instead.
        if (isset($CFG->kilman_usergraph)) {
            set_config('usergraph', $CFG->kilman_usergraph, 'kilman');
            unset_config('kilman_usergraph');
        }
        upgrade_mod_savepoint(true, 2015051101, 'kilman');
    }

    // Add index to reduce load on the kilman_quest_choice table.
    if ($oldversion < 2015051102) {
        // Conditionally add an index to the question_id field.
        $table = new xmldb_table('kilman_quest_choice');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2015051102, 'kilman');
    }
    ...
    if ($oldversion < 2016020204) {
        ...
        $table = new xmldb_table('kilman_response_date');
        $field = new xmldb_field('response', XMLDB_TYPE_TEXT, null, null, null, null, null);
        $dbman->change_field_precision($table, $field);

        // kilman savepoint reached.
         upgrade_mod_savepoint(true, 2016020204, 'kilman');
    }
    ...
    if ($oldversion < 2016111105) {
        ...
        // kilman savepoint reached.
         upgrade_mod_savepoint(true, 2016111105, 'kilman');
    }
    ...
    if ($oldversion < 2017050101) {
        ...
        if ($dbman->field_exists($table, $field)) {
            ...
            $sql = 'SELECT qr.id, qr.username, qa.rid, qa.userid ' .
                   ...
                   'INNER JOIN {kilman_attempts} qa ON qr.id = qa.rid ' .
                   'WHERE qr.username = ?';
            $rs = $DB->get_recordset_sql($sql, ["Anonymous"]);
            // Set all "Anonymous" records to the userid in the matching attempt record.
            foreach ($rs as $record) {
                $DB->set_field('kilman_response', 'username', "{$record->userid}", ['id' => $record->id]);
            }
            // If there are any leftover "Anonymous" records, set them all to userid zero (there shouldn't be).
            $rs = $DB->get_recordset('kilman_response', ['username' => 'Anonymous']);
            foreach ($rs as $record) {
                $DB->set_field('kilman_response', 'username', '0', ['id' => $record->id]);
            }
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2017050101, 'kilman');
    }

    // Converting to new dependency system.
    if ($oldversion < 2017111101) {
        // MOD Multiparent Advanceddependencies START.
        // Define table kilman_dependency to be created.
        $table = new xmldb_table('kilman_dependency');

        // Adding fields to table kilman_depenencies.
        ...
        // Adding keys to table kilman_depenencies.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, array('id'));

        // Adding indexes to table kilman_dependency.
        $table->add_index('quest_dependency_quesidx', XMLDB_INDEX_NOTUNIQUE, array('questionid'));

        // Conditionally launch create table for kilman_dependencies.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);

            // Copy all existing branching data into new branching structure.
            $branchingrs = $DB->get_recordset_select('kilman_question', 'dependquestion > 0 AND deleted = \'n\'',
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2017111101, 'kilman');
    }
    ...
    if ($oldversion < 2017111103) {
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2017111103, 'kilman');
    }

    // Get rid of kilman_attempts table and migrate necessary data to the kilman_response table.
    if ($oldversion < 2018050102) {
        $table = new xmldb_table('kilman_response');
        $field1 = new xmldb_field('kilmanid', XMLDB_TYPE_INTEGER, '10', null, XMLDB_NOTNULL, null, '0', 'id');
        $field2 = new xmldb_field('survey_id');

        // Create the new kilmanid field, if it doesn't already exist (it shouldn't).
        if (!$dbman->field_exists($table, $field1)) {
            $dbman->add_field($table, $field1);
        }

        // Get all of the attempts records, and add the kilman id to the corresponding response record.
        $rs = $DB->get_recordset('kilman_attempts');
        foreach ($rs as $attempt) {
            $DB->set_field('kilman_response', 'kilmanid', $attempt->qid, ['id' => $attempt->rid]);
        }
        $rs->close();

        // Get all of the response records with a '0' kilmanid, and extract the kilmanid from the survey_id field.
        $rs = $DB->get_recordset('kilman_response', ['kilmanid' => 0]);
        foreach ($rs as $response) {
            if ($kilman = $DB->get_record('kilman', ['sid' => $response->survey_id], 'id,sid', IGNORE_MULTIPLE)) {
                $DB->set_field('kilman_response', 'kilmanid', $kilman->id, ['id' => $response->id]);
            }
        }
        $rs->close();
        ...
        // Add an index for the new kilmanid field.
        $index = new xmldb_index('kilmanidx');
        $index->set_attributes(XMLDB_INDEX_NOTUNIQUE, ['kilmanid']);
        $dbman->add_index($table, $index);

        // Now drop the unnecessary attempts table.
        $table = new xmldb_table('kilman_attempts');
        $dbman->drop_table($table);

        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2018050102, 'kilman');
    }

    // Rename the mdl_kilman_response_rank.rank field as it is reserved in MySQL as of 8.0.2. This step may have already
    ...
    if ($oldversion < 2018050104) {
        ...
        if ($DB->get_dbfamily() !== 'mysql') {
            ...
            if ($dbman->field_exists('kilman_response_rank', 'rank')) {
                ...
                $sql = 'ALTER TABLE {kilman_response_rank} ' .
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2018050104, 'kilman');
    }
    ...
    if ($oldversion < 2018050105) {
        ...
        $DB->set_field_select('kilman_survey', 'feedbacksections', 2, 'feedbacksections > 2');
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2018050105, 'kilman');
    }
    ...
    if ($oldversion < 2018050106) {
        $table1 = new xmldb_table('kilman_fb_sections');
        $field1 = new xmldb_field('survey_id', XMLDB_TYPE_INTEGER, '18');
        $table2 = new xmldb_table('kilman_feedback');
        $field2 = new xmldb_field('section_id', XMLDB_TYPE_INTEGER, '18');
        $table3 = new xmldb_table('kilman_question');
        ...
        // kilman savepoint reached.
        upgrade_mod_savepoint(true, 2018050106, 'kilman');
    }
...
function kilman_upgrade_2007120101() {
    global $DB;
    ...
    $qtable = new xmldb_table('kilman_question_choice');
    $dbman->rename_table($qtable, 'kilman_quest_choice', false);
    unset($qtable);

    $qtable = new xmldb_table('kilman_response_multiple');
    $dbman->rename_table($qtable, 'kilman_resp_multiple', false);
    unset($qtable);

    $qtable = new xmldb_table('kilman_response_single');
    $dbman->rename_table($qtable, 'kilman_resp_single', false);
    unset($qtable);

    // Upgrade the kilman_question_type table to use typeid.
    $table = new xmldb_table('kilman_question_type');
    $field = new xmldb_field('typeid');
    $field->set_attributes(XMLDB_TYPE_CHAR, '20', true, true, false, false, null, '0', 'id');
    $dbman->add_field($table, $field);
    if (($numrecs = $dbman->count_records('kilman_question_type')) > 0) {
        $recstart = 0;
        $recstoget = 100;
        while ($recstart < $numrecs) {
            if ($records = $dbman->get_records('kilman_question_type', array(), '', '*', $recstart, $recstoget)) {
                foreach ($records as $record) {
                    $dbman->set_field('kilman_question_type', 'typeid', $record->id, array('id' => $record->id));
                }
            }
            $recstart += $recstoget;
        }
