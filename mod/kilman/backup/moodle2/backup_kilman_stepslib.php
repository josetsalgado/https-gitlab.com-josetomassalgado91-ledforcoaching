...
/**
 * Define all the backup steps that will be used by the backup_kilman_activity_task
 */

/**
 * Define the complete choice structure for backup, with file and id annotations
 */
class backup_kilman_activity_structure_step extends backup_activity_structure_step {
    ...
    protected function define_structure() {
        ...
        $kilman = new backup_nested_element('kilman', array('id'), array(
        ...
        $response = new backup_nested_element('response', array('id'), array(
            'kilmanid', 'submitted', 'complete', 'grade', 'userid'));
        ...
        // Define sources.
        $kilman->set_source_table('kilman', array('id' => backup::VAR_ACTIVITYID));

        // Is current kilman based on a public kilman?
        $qid = $this->task->get_activityid();
        $currentkilman = $DB->get_record("kilman", array ("id" => $qid));
        $currentsurvey = $DB->get_record("kilman_survey", array ("id" => $currentkilman->sid));
        $haspublic = false;
        if ($currentsurvey->realm == 'public' && $currentsurvey->courseid != $currentkilman->course) {
            $haspublic = true;
        }

        // If current kilman is based on a public one, do not include survey nor questions in backup.
        if (!$haspublic) {
            $survey->set_source_table('kilman_survey', array('id' => '../../sid'));
            $question->set_source_table('kilman_question', array('surveyid' => backup::VAR_PARENTID));
            $fbsection->set_source_table('kilman_fb_sections', array('surveyid' => backup::VAR_PARENTID));
            $feedback->set_source_table('kilman_feedback', array('sectionid' => backup::VAR_PARENTID));
            $questchoice->set_source_table('kilman_quest_choice', array('question_id' => backup::VAR_PARENTID));
            $questdependency->set_source_table('kilman_dependency', array('questionid' => backup::VAR_PARENTID));

            // All the rest of elements only happen if we are including user info.
            if ($userinfo) {
                $response->set_source_table('kilman_response', array('kilmanid' => backup::VAR_PARENTID));
                $responsebool->set_source_table('kilman_response_bool', array('response_id' => backup::VAR_PARENTID));
                $responsedate->set_source_table('kilman_response_date', array('response_id' => backup::VAR_PARENTID));
                $responsemultiple->set_source_table('kilman_resp_multiple', array('response_id' => backup::VAR_PARENTID));
                $responseother->set_source_table('kilman_response_other', array('response_id' => backup::VAR_PARENTID));
                $responserank->set_source_table('kilman_response_rank', array('response_id' => backup::VAR_PARENTID));
                $responsesingle->set_source_table('kilman_resp_single', array('response_id' => backup::VAR_PARENTID));
                $responsetext->set_source_table('kilman_response_text', array('response_id' => backup::VAR_PARENTID));
            }
        ...
        $kilman->annotate_files('mod_kilman', 'intro', null); // This file area hasn't itemid.

        $survey->annotate_files('mod_kilman', 'info', 'id'); // By survey->id
        $survey->annotate_files('mod_kilman', 'thankbody', 'id'); // By survey->id.

        $question->annotate_files('mod_kilman', 'question', 'id'); // By question->id.

        $fbsection->annotate_files('mod_kilman', 'sectionheading', 'id'); // By feedback->id.
        $feedback->annotate_files('mod_kilman', 'feedback', 'id'); // By feedback->id.

        // Return the root element, wrapped into standard activity structure.
        return $this->prepare_activity_structure($kilman);
    }
}