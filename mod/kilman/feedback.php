...
$PAGE->set_url(new moodle_url($CFG->wwwroot.'/mod/kilman/feedback.php', ['id' => $id]));
...
$feedbackform = new \mod_kilman\feedback_form('feedback.php');
$sdata = clone($kilman->survey);
$sdata->sid = $kilman->survey->id;
$sdata->id = $cm->id;

$draftideditor = file_get_submitted_draft_itemid('feedbacknotes');
$currentinfo = file_prepare_draft_area($draftideditor, $context->id, 'mod_kilman', 'feedbacknotes',
    $sdata->sid, ['subdirs' => true], $kilman->survey->feedbacknotes);
...
if ($feedbackform->is_cancelled()) {
    redirect(new moodle_url('/mod/kilman/view.php', ['id' => $kilman->cm->id]));
}
// Confirm that feedback can be used for this kilman...
...
if ($settings = $feedbackform->get_data()) {
    if (isset($settings->feedbacksettingsbutton1) || isset($settings->buttongroup)) {
        ...
        if (isset ($settings->feedbacknotes)) {
            ...
            $sdata->feedbacknotes = file_save_draft_area_files($sdata->fbnotesitemid, $context->id, 'mod_kilman',
    ...
    if (isset($settings->buttongroup['feedbackeditbutton'])) {
        ...
        if (!($firstsection = $DB->get_field('kilman_fb_sections', 'MIN(section)', ['surveyid' => $kilman->sid]))) {
            $firstsection = 0;
        }
        ...
        redirect(new moodle_url('/mod/kilman/fbsections.php', ['id' => $cm->id, 'section' => $firstsection]));
    }
}
...
require('tabs.php');
if (!$validquestions) {
    $kilman->page->add_to_page('formarea', get_string('feedbackoptions_help', 'kilman'));
} else {
